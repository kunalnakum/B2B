...
function AddProduct() {
    ...
    const handleApi = () => {
        const categoryKeywords = {
            'Chemical': ['chemical', 'chemistry', 'lab', 'experiment'],
            'Iron': ['iron', 'metal', 'steel', 'cast iron'],
            'Brass': ['brass', 'metal', 'alloy'],
            'Wood': ['wood', 'timber', 'lumber'],
            'Steel': ['steel', 'metal', 'alloy'],
            'Electronics': ['electronics', 'device', 'gadget'],
            'Construction': ['construction', 'building', 'material'],
            'Drugs & Pharma': ['drug', 'pharma', 'medicine', 'health'],
            'Plastic Products': ['plastic', 'polymer', 'resin']
        };
    
        // Check if product name matches the selected category
        const categoryKeywordsLowerCase = categoryKeywords[category].map(keyword => keyword.toLowerCase());
        const productNameLowerCase = pname.toLowerCase();
    
        const matchedKeyword = categoryKeywordsLowerCase.find(keyword => productNameLowerCase.includes(keyword));
    
        if (!matchedKeyword) {
            alert('Product name should match the selected category.');
